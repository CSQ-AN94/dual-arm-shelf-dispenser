...
class BottlePickPlaceTask:
    ...
    def run(
        ...
        # DISPENSE is gated by the delivery profile, not by a hard refusal
        # here.  load_safety_profile already rejects `enabled: false`, and
        # rejects `verified_for_execution: false` under --execute; then
        # MobileBodyCoordinator._validated_shelf_ready fails closed on every
        # null field and every false `*_verified` in side_table_delivery.
        # A third refusal at the entry only hid which of those is missing.
