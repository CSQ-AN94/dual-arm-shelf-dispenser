# shelf_dispenser — 双臂货架售货抓取机器人（技术验证中）
...
`shelf_dispenser/` 是一套完全独立的新 SDK 控制栈（不碰旧的
...
python -m pytest test/shelf_dispenser/
```
...
scripts/run_task.sh from-pregrasp

# 右臂已经在腕部观察位
scripts/run_task.sh from-observation

# 从头部定位开始完整跑一遍（含返回初始姿态）
scripts/run_task.sh from-start
```
...
合并进 `shelf_dispenser/safety_profiles.json` 的 `shelf_template`（id 必须是
...
上机前的规划彩排入口是 `scripts/run_run_task.sh`：它用真实场景和
规划链路生成证据，但不执行机械臂运动。
...
test/shelf_dispenser/   280个测试，纯逻辑+mock，不连真机/ROS
```
