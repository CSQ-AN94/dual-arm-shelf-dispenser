...
from shelf_dispenser.workflows import (
    BottlePickPlaceTask,
    DeliverMode,
    ObjectState,
    RunStatus,
...
def _wire_real_shelf_ready_gate(demo, tmp_path, *, chassis_yaw_deg):
    """Use RunOrchestrator's real pre-arm body admission with inert adapters."""
    ...
    def capture():
        ...
        snapshot = RunOrchestrator._capture_shelf_ready_for_dispense(demo)
...
def test_stop_after_with_body_guard_keeps_the_gripper_untouched(tmp_path):
    ...
    demo._preflight = lambda: RunOrchestrator._preflight(demo)
...
def test_two_runs_started_at_the_same_timestamp_get_distinct_evidence_dirs(
    ...
    first = RunOrchestrator(args, None)
    second = RunOrchestrator(args, None)
    try:
