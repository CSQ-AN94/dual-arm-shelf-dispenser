...
surface depth below come from ``outputs/bottle_grasp/20260718_203207``.  The
...
def test_bottom_clipped_wrist_box_keeps_locked_target_depth(monkeypatch):
    ...
    demo.detector = None
    monkeypatch.setattr("bottle_grasp.demo.time.sleep", lambda _: None)
...
def test_independent_head_measurement_never_synthesizes_prior_depth(monkeypatch):
    ...
    monkeypatch.setattr("bottle_grasp.demo.time.time", lambda: next(clock))
