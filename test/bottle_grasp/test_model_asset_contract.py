...
from bottle_grasp.model_assets import (
    ModelAssetContractError,
    inspect_model_asset_contract,
    require_model_asset_contract,
)
from bottle_grasp.run_manifest import build_run_manifest
...
def _archive_release(
    ...
    *,
    ...
    (source / "bottle_grasp").mkdir()
    (source / "bottle_grasp" / "model_assets.lock.json").write_text(
...
def test_asset_lock_rejects_wrong_contract_or_duplicate_roles(
    ...
    deployment = _archive_release(tmp_path)
    lock_path = deployment / "bottle_grasp" / "model_assets.lock.json"
...
def test_demo_rejects_missing_primary_before_safety_or_hardware_setup(
    ...
    monkeypatch.setattr(
        "bottle_grasp.demo.load_safety_profile",
...
def test_initialize_passes_verified_absolute_primary_to_detector_after_chdir(
    ...
    monkeypatch.setattr("bottle_grasp.demo.Dashboard", FakeDashboard)
    monkeypatch.setattr("bottle_grasp.demo.BottleDetector", fake_detector)
...
def test_wrist_uses_verified_primary_when_optional_fallback_is_unavailable(
    ...
    monkeypatch.setattr(
        "bottle_grasp.demo.BottleDetector",
